...
import {
  getPropertyById,
  updateProperty,
  enrichProperty as enrichPropertyService,
  enrichPropertyRentcast as enrichPropertyRentcastService,
  getMapsConfig,
...
function PropertyStreetView({
  ...
  const streetViewUrl = getStreetViewImageUrl(fullAddress, "640x300");
...
function PropertyMap({
  ...
  useEffect(() => {
    async function fetchKey() {
      try {
        const response = await getMapsConfig();
...
export default function PropertyProfilePage() {
  ...
  useEffect(() => {
    if (property && editedProperty) {
      setHasChanges(
        JSON.stringify(property) !== JSON.stringify(editedProperty)
      );
    }
  ...
  async function saveProperty() {
    ...
    setSaving(true);
    try {
      const response = await updateProperty(id, editedProperty);
  ...
  async function enrichProperty() {
    ...
    setEnriching(true);
    try {
      await enrichPropertyService(id);
      setTimeout(() => {
        fetchProperty();
        setEnriching(false);
      }, 2000);
  ...
  async function enrichRentcast() {
    ...
    try {
      const response = await enrichPropertyRentcastService(id);
      setTimeout(() => {
        fetchProperty();
      ...
      if (error.response?.status === 429) {
        ...
        alert(
          data.message ||
            "RentCast limit reached. Only 10 properties can be synced."
        );
      } else {
      ...
      case "rate_limited":
        return (
          <Badge className="bg-yellow-100 text-yellow-700">
            <Clock className="w-3 h-3 mr-1" /> Rate Limited
          </Badge>
        );
      default:
        return (
        ...
        <div className="flex flex-col md:flex-row md:items-start justify-between gap-4">
          ...
          <div className="flex items-center gap-3">
            ...
            {hasChanges && (
              ...
              >
                <Save
                  className={`w-4 h-4 mr-2 ${saving ? "animate-pulse" : ""}`}
                />
            ...
            {property.attomStatus !== "success" && (
              <Button
                variant="outline"
                onClick={enrichProperty}
                disabled={enriching}
                data-testid="button-enrich"
              >
                <RefreshCw
                  className={`w-4 h-4 mr-2 ${enriching ? "animate-spin" : ""}`}
                />
                ...
                {editedProperty.attomAvmValue && (
                  <div className="bg-gradient-to-r from-[#1C49A6]/10 to-[#99C054]/10 rounded-lg p-4 border border-[#1C49A6]/20">
                    <div className="flex items-center justify-between mb-2">
                      <span className="text-sm font-medium text-[#1C49A6]">
                        Investee Estimated Property Value
                      </span>
                    ...
                    {(editedProperty.attomAvmLow ||
                      editedProperty.attomAvmHigh) && (
                      <div className="text-sm text-muted-foreground mt-1">
                        Range: $
                        {editedProperty.attomAvmLow?.toLocaleString() || "N/A"}{" "}
                        - $
                        {editedProperty.attomAvmHigh?.toLocaleString() || "N/A"}
                      </div>
                    )}
                  </div>
                )}
                    ...
                    <div className="flex items-center justify-between">
                      <div>
                        <span className="text-sm font-medium text-amber-700">
                          Investee Estimated Annual Taxes
                        </span>
                ...
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateField("ownerOccupied", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateField("listedForSale", e.target.value)
                      }
                      className="w-full border rounded-md px-3 py-2 text-sm bg-background"
                  ...
                  <div>
                    ...
                    <select
                      ...
                      onChange={(e) =>
                        updateField("foreclosure", e.target.value)
                      }
                      className="w-full border rounded-md px-3 py-2 text-sm bg-background"
            ...
            <Card>
              ...
              <CardContent className="space-y-4">
                <div className="grid grid-cols-2 gap-4">
                  ...
                  <div>
                    <Label className="text-sm text-muted-foreground">
                      ATTOM Property Class
                    </Label>
                    <Input
                      value={editedProperty.attomPropClass || ""}
                      onChange={(e) =>
                        updateField("attomPropClass", e.target.value)
                      }
        ...
        <TabsContent value="rentcast" className="mt-6">
            ...
            >
              <RefreshCw
                className={`w-4 h-4 mr-2 ${
                  enrichingRentcast ? "animate-spin" : ""
                }`}
              />
          ...
          {property.rentcastStatus === "success" ? (
            <div className="space-y-6">
              {property.rentcastPropertyData && (
                <Card>
                  ...
                  <CardContent>
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {property.rentcastPropertyData.bedrooms != null && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-lg font-semibold">
                            {property.rentcastPropertyData.bedrooms}
                          </div>
                      ...
                      {property.rentcastPropertyData.bathrooms != null && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-lg font-semibold">
                            {property.rentcastPropertyData.bathrooms}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Bathrooms
                          </div>
                      ...
                      {property.rentcastPropertyData.squareFootage != null && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-lg font-semibold">
                            {property.rentcastPropertyData.squareFootage.toLocaleString()}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Sq Ft
                          </div>
                      ...
                      {property.rentcastPropertyData.yearBuilt != null && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-lg font-semibold">
                            {property.rentcastPropertyData.yearBuilt}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Year Built
                          </div>
                      ...
                      {property.rentcastPropertyData.lotSize != null && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-lg font-semibold">
                            {property.rentcastPropertyData.lotSize.toLocaleString()}
                          </div>
                          <div className="text-xs text-muted-foreground">
                            Lot Size (sq ft)
                          </div>
                            ...
                            <div className="flex flex-wrap gap-2">
                              ...
                              {f.coolingType && (
                                <Badge variant="outline">
                                  {f.coolingType} Cooling
                                </Badge>
                              )}
                              {f.heatingType && (
                                <Badge variant="outline">
                                  {f.heatingType} Heating
                                </Badge>
                              )}
                              {f.architectureType && (
                                <Badge variant="outline">
                              ...
                              {f.foundationType && (
                                <Badge variant="outline">
                                  {f.foundationType} Foundation
                                </Badge>
                              )}
                              {f.exteriorType && (
                                <Badge variant="outline">
                                  {f.exteriorType} Exterior
                                </Badge>
                              )}
                              {f.parkingSpaces && (
                                <Badge variant="outline">
                                  {f.parkingSpaces} Parking Spaces
                                </Badge>
                              )}
                              {Array.isArray(f.flooringTypes) &&
                                f.flooringTypes.map(
                                  (floor: string, idx: number) => (
                    ...
                    {property.rentcastPropertyData.owner?.names &&
                      property.rentcastPropertyData.owner.names.length > 0 && (
                        <div className="mt-4 pt-4 border-t">
                          <div className="text-sm font-medium mb-1">Owner</div>
                          <div className="text-muted-foreground">
                            {property.rentcastPropertyData.owner.names.join(
                              ", "
                            )}
                          </div>
                        </div>
                    ...
                    {property.rentcastValueEstimate ? (
                      <div className="bg-gradient-to-r from-purple-50 to-blue-50 rounded-lg p-4 border border-purple-200">
                        <div className="text-sm font-medium text-purple-700 mb-1">
                          RentCast Estimated Value
                        </div>
                        <div className="text-3xl font-bold text-purple-800">
                          ${property.rentcastValueEstimate.toLocaleString()}
                        </div>
                        {(property.rentcastValueLow ||
                          property.rentcastValueHigh) && (
                          <div className="text-sm text-muted-foreground mt-1">
                            Range: $
                            {property.rentcastValueLow?.toLocaleString() ||
                              "N/A"}{" "}
                            - $
                            {property.rentcastValueHigh?.toLocaleString() ||
                              "N/A"}
                          </div>
                        )}
                      </div>
                    ...
                    {property.rentcastRentEstimate ? (
                      <div className="bg-gradient-to-r from-green-50 to-emerald-50 rounded-lg p-4 border border-green-200">
                        <div className="text-sm font-medium text-green-700 mb-1">
                          RentCast Monthly Rent
                        </div>
                        <div className="text-3xl font-bold text-green-800">
                          ${property.rentcastRentEstimate.toLocaleString()}/mo
                        </div>
                        {(property.rentcastRentLow ||
                          property.rentcastRentHigh) && (
                          <div className="text-sm text-muted-foreground mt-1">
                            Range: $
                            {property.rentcastRentLow?.toLocaleString() ||
                              "N/A"}{" "}
                            - $
                            {property.rentcastRentHigh?.toLocaleString() ||
                              "N/A"}
                          </div>
                        )}
                      </div>
              ...
              {property.rentcastPropertyData?.taxAssessments &&
                typeof property.rentcastPropertyData.taxAssessments ===
                  "object" &&
                Object.keys(property.rentcastPropertyData.taxAssessments)
                  .length > 0 && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
              ...
              {property.rentcastSaleComps &&
                Array.isArray(property.rentcastSaleComps) &&
                property.rentcastSaleComps.length > 0 && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
              ...
              {property.rentcastRentComps &&
                Array.isArray(property.rentcastRentComps) &&
                property.rentcastRentComps.length > 0 && (
                  <Card>
                    <CardHeader>
                      <CardTitle className="flex items-center gap-2">
                    ...
                    <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
                      {property.rentcastMarketData.medianSalePrice && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-lg font-semibold">
                            $
                            {property.rentcastMarketData.medianSalePrice.toLocaleString()}
                          </div>
                        </div>
                      )}
                      {property.rentcastMarketData.medianRent && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-lg font-semibold">
                            $
                            {property.rentcastMarketData.medianRent.toLocaleString()}
                            /mo
                          </div>
                        </div>
                      )}
                      ...
                      {property.rentcastMarketData.saleToListRatio && (
                        <div className="text-center p-3 bg-muted rounded-lg">
                          ...
                          <div className="text-lg font-semibold">
                            {(
                              property.rentcastMarketData.saleToListRatio * 100
                            ).toFixed(1)}
                            %
                          </div>
                        </div>
            ...
            <Card className="bg-muted/50">
              <CardContent className="py-12 text-center">
                ...
                <h3 className="text-lg font-medium mb-2">
                  No RentCast Data Yet
                </h3>
                <p className="text-muted-foreground mb-4">
                  Click "Fetch RentCast Data" to get property value estimates,
                  rent estimates, comparables, tax history, and market data.
                </p>
